...
NOTE: The value supplied to the `projection` query parameter is the same as specified in `@Projection(name = "noAddresses")`. It has nothing to do with the name of the projection's interface.
...
How does Spring Data REST find projection definitions?
